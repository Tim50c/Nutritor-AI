...
export default function GoalWeightScreen() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Update local context with the new onboarding status
      if (userProfile) {
        setUserProfile({
          ...userProfile,
          dob,
          gender: data.gender,
          weightCurrent: data.weightCurrent?.toString() || null,
          weightGoal: data.weightGoal?.toString() || null,
          onboardingComplete: true,
        });
      }

      // Navigate to main app since onboarding is complete
      router.replace("/(tabs)");
    } catch (error: any) {
