  ...
  <GestureHandlerRootView style={{ flex: 1 }}>
    <SafeAreaView className="flex-1 bg-white dark:bg-black">
      <View className="flex-row items-center justify-between px-4 py-3 bg-white dark:bg-black">
        ...
        <Text className="text-xl font-bold text-black dark:text-white">Search</Text>
          ...
          <View className="px-4 pt-6">
            <View className="flex-row items-center mb-4">
              <View className="flex-1 flex-row items-center bg-gray-100 dark:bg-gray-800 rounded-xl px-3 h-12 mr-3">
                 <Ionicons name="search" size={22} color={isDark ? "#A0AEC0" : "#9CA3AF"} />
                <TextInput
                ...
                {searchText.length > 0 && (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                  >
                    <Ionicons name="close-circle" size={20} color={isDark ? "#A0AEC0" : "#9CA3AF"} />
                  </TouchableOpacity>
                )}
              </View>
              ...
              <TouchableOpacity
                ...
                className={`w-12 h-12 items-center justify-center rounded-xl ${
                  hasActiveFilters ? 'bg-orange-500' : 'bg-gray-100 dark:bg-gray-800'
                }`}
                activeOpacity={0.8}
              >
                <Ionicons 
                  ...
                  size={24} 
                  color={hasActiveFilters ? 'white' : (isDark ? 'white' : 'black')} 
                />
              </TouchableOpacity>
            </View>
            ...
            {showFilters && (
              ...
              >
                <View className="flex-row justify-between items-center mb-3">
                  <Text className="text-lg font-bold text-gray-800 dark:text-gray-100">Filter by Nutrients</Text>
                  <TouchableOpacity onPress={resetFilters} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded-lg">
                    <Text className="text-sm text-gray-600 dark:text-gray-400">Reset</Text>
                  </TouchableOpacity>
                </View>
                <NutrientSlider 
          ...
          <View>
            {isSearching && (
              <View className="items-center justify-center py-20">
                ...
                <Text className="text-gray-500 dark:text-gray-400 text-base mt-4">Searching foods...</Text>
              </View>
            )}
            ...
            {showInitialState && (
              <AnimatableView animation="fadeIn" duration={500} className="items-center justify-center py-16">
                ...
                <Text className="text-gray-500 dark:text-gray-400 text-base text-center mt-4">Search for food by name or filter by nutrients</Text>
                <Text className="text-gray-400 dark:text-gray-500 text-sm mt-1">Try searching for &quot;chicken&quot;, &quot;apple&quot;, or use filters</Text>
                <View className="px-4 mt-12 w-full">
                  <Text className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3">Quick Searches</Text>
                  <View className="flex-row flex-wrap">
                    {['chicken', 'rice', 'apple', 'salmon', 'broccoli', 'banana'].map((suggestion) => (
                      <TouchableOpacity
                        key={suggestion}
                        onPress={() => setSearchText(suggestion)}
                        className="bg-gray-100 dark:bg-gray-700 px-4 py-2 rounded-full mr-2 mb-2"
                      >
                        <Text className="text-gray-700 dark:text-gray-300 capitalize">{suggestion}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </View>
            ...
            {showNoResults && (
              <AnimatableView animation="fadeIn" duration={500} className="items-center justify-center py-20 px-4">
                ...
                <Text className="text-gray-500 dark:text-gray-400 text-base text-center mt-4">{fallbackText}</Text>
                <Text className="text-gray-400 dark:text-gray-500 text-sm text-center mt-2">Try a different search term or adjust filters.</Text>
              </AnimatableView>
            )}
              ...
              <AnimatableView ref={resultsViewRef} animation="fadeInUp" duration={500} className="px-4">
                <View className="mb-4">
                  ...
                  <View className="flex-row items-center">
                    <Text className="text-sm text-gray-600 dark:text-gray-400 mr-2">Sort:</Text>
                          ...
                          return (
                            <TouchableOpacity key={option.key} onPress={() => handleSort(option.key)} activeOpacity={0.8}
                                className={`px-3 py-1 rounded-lg mr-2 ${
                                  isActive ? 'bg-orange-100 dark:bg-orange-900' : 'bg-gray-200 dark:bg-gray-700'
                                }`}>
                              <Text className={`text-sm font-medium ${
                                isActive ? 'text-orange-600 dark:text-orange-300' : 'text-gray-700 dark:text-gray-300'
                              }`}>
                ...
                {hasMoreResults && (
                  <TouchableOpacity 
                    ...
                    className="mx-4 my-4 py-3 bg-orange-50 dark:bg-orange-900 border border-orange-200 dark:border-orange-800 rounded-lg"
                    activeOpacity={0.7}
                  >
  ...
  return (
    <View className="mb-4">
      <View className="flex-row justify-between items-center mb-2">
        <Text className="text-base text-gray-700 dark:text-gray-300 font-medium">{label}</Text>
