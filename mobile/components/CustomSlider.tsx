...
const CustomSlider: React.FC<CustomSliderProps> = ({
  ...
  const activeTrackColorDark = '#ff5a16'; // Using the same orange as it works well in both themes
  const thumbColorDark = '#ff5a16'; // Same as active track
  ...
  const finalTrackColor = isDark ? trackColorDark : trackColor;
  const finalActiveTrackColor = isDark ? activeTrackColorDark : activeTrackColor;
  const finalThumbColor = isDark ? thumbColorDark : thumbColor;

  return (
    <View style={[styles.container, style]} onLayout={handleLayout}>
      <View style={[styles.track, { backgroundColor: finalTrackColor, height: TRACK_HEIGHT, marginLeft: THUMB_SIZE / 2 }]} />
      <Animated.View style={[styles.activeTrack, { backgroundColor: finalActiveTrackColor, height: TRACK_HEIGHT, marginLeft: THUMB_SIZE / 2 }, activeTrackAnimatedStyle]} />
      <PanGestureHandler onGestureEvent={gestureHandler}>
        <Animated.View style={[styles.thumb, { backgroundColor: finalThumbColor, width: THUMB_SIZE, height: THUMB_SIZE, borderRadius: THUMB_SIZE / 2 }, thumbAnimatedStyle]} />
      </PanGestureHandler>
    </View>
  );
};
