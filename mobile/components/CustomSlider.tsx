...
const CustomSlider: React.FC<CustomSliderProps> = ({
  ...
  onSlidingComplete,
  style,
  trackColor = "bg-border-default dark:bg-border-default-dark",
  activeTrackColor = "bg-accent dark:bg-accent-dark",
  thumbColor = "bg-accent dark:bg-accent-dark",
}) => {
  ...
  >({
    ...
    onActive: (event, context) => {
      ...
      const newPosition = Math.max(
        0,
        Math.min(trackWidth, context.startX + event.translationX)
      );
      ...
      if (
        Math.abs(newValue - context.lastValue) > 0.5 &&
